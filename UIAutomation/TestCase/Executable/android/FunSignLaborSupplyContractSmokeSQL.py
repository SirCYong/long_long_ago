...
'''
   ...
   签订劳务契约恢复数据
'''
from UIAutomation.TestCase.SIT.Android.SmokeTesting.FunSmoke004SQL import get_creator_ukid
# from UIAutomation.Utils import basic_cit, close_oracle
from UIAutomation.Utils import basic_sit, close_oracle


def delete_labor_contract(user_id=10001475, participant_ukid=10001465, contract_type='LA'):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    # 删除劳务契约
    try:
...
# 获取劳务契约ukid
def get_labor_contract_ukid(participant_ukid =10001465 , contract_type='LA'):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
...
def get_new_labor_contract(user_id, contract_ukid):
    ...
    try:
        ...
        sql4 = '''select count(*) from xdw_app.ru_labour_log where accept_id in ('%s', '%s') and CONTRACT_SOURCE_UKID = '%s' ''' \
               % (demander_id, creator_ukid, contract_ukid)
        print(sql4)
        curs.execute(sql4)
