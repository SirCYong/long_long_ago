...
(select t.card_ukid from xdw_app.ms_card t where t.owner_bu_id = '%s' and t.card_name = '激活实体店')''' % user_id,
...
t.create_user_id = '%s'  and t.item_type = '实体店'))''' % user_id,
            ...
            from basic_owner.ba_sales_building t where t.item_ukid =(select t.item_ukid from basic_owner.IM_ITEM t where
            t.create_user_id = '%s'  and t.item_type = '实体店'))''' % user_id,
            '''delete from basic_owner.IM_ITEM t where t.create_user_id = '%s'  and t.item_type = '实体店' ''' % user_id
               ]
        for sql1 in sql:
            curs.execute(sql1)
            con.commit()
