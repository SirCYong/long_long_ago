...
# 删除运输契约
def delete_transport_contract(user_id=10001476, participant_ukid =10001476, contract_type='TR'):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
        creator_ukid = get_creator_ukid(user_id)
        sql2 = ['''delete from xdw_app.dd_contract where supplier_id  in (select supplier_id from
...
# 获取运输契约ukid
def get_transport_contract_ukid(participant_ukid= 10001429, contract_type='TR'):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
...
def get_transport_contract_info(user_id, contract_ukid, contract_item_ukid ='51817000000009223'):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
        creator_ukid = get_creator_ukid(user_id)
...
def get_new_transport_contract(user_id, contract_ukid=51817000000009223):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
        dic = {}
        ...
        sql2 = ''' select count(*) from xdw_app.dd_contract where supplier_id  in (select supplier_id from
                    xdw_app.dd_contract where CONTRACT_UKID =  '%s') and demander_id ='%s' and contract_type='TR' order by
        ...
        sql3 = '''select count(*) from xdw_app.rs_repository  where rs_type='TR' and owner_id in ('%s', '%s') '''% (supplier_id, creator_ukid)
        print(sql3)
        curs.execute(sql3)
        result3 = curs.fetchall()
        dic['rs_repository_no'] = result3[0][0]

        sql4 = '''select count(*) from xdw_app.ru_transport_supply_log where log_user_id = '%s'
                  and CONTRACT_SOURCE_UKID = '%s' ''' \
               % (creator_ukid, contract_ukid)
        print(sql4)
        curs.execute(sql4)
...
# print(get_new_transport_contract(199335, contract_ukid=51765800000034214))
# print(reduction_transport_contract())
