'''
    author: zhoujin
   管理业务单位恢复数据，恢复到只有一级业务单位
'''
# from UIAutomation.Utils import basic_cit, close_oracle
from UIAutomation.Utils import basic_sit, close_oracle
# from UIAutomation.TestCase.SIT.iOS.SmokeTesting.FunSmoke014SQL import get_creator_ukid
# 删除管理业务单位
...
# 获取劳务契约ukid
def get_labor_contract_ukid(participant_ukid, contract_type='LA'):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
        sql = '''select CONTRACT_UKID from xdw_app.dd_contract where CONTRACT_TYPE = '%s' and
                 DEMANDER_ID='%s' order by create_time desc ''' % (contract_type, participant_ukid)
        curs.execute(sql)
...
def get_new_labor_contract(user_id, contract_ukid):
    # con, curs = basic_cit()
    con, curs = basic_sit()
    try:
        dic = {}
        ...
        sql2 = '''select count(*) from xdw_app.dd_contract where demander_id  in (select demander_id from
                     xdw_app.dd_contract where CONTRACT_UKID =  '%s') and supplier_id ='%s' and contract_type='LA' order by
        ...
        sql3 = '''select count(*) from xdw_app.rs_repository  where rs_type='LA' and owner_id in ('%s', '%s') '''% (demander_id, creator_ukid)
        print(sql3)
        curs.execute(sql3)
        result3 = curs.fetchall()
        dic['rs_repository_no'] = result3[0][0]

        sql4 = '''select count(*) from xdw_app.ru_labour_log where accept_id='%s' and CONTRACT_SOURCE_UKID = '%s' ''' \
...
def reduction_labor_contract(contract_ukid=51817400000009165):
    """
    还原劳务需求契约
    """
    # con, curs = basic_cit()
    ...
    try:
        sql_list = ['''update xdw_app.dd_contract set contract_status=20 where contract_ukid = '%s' ''' % contract_ukid,
                    '''update xdw_app.dd_contract_item set qty = 1 where contract_ukid = '%s' ''' % contract_ukid]
        for sql in sql_list:
            curs.execute(sql)
            con.commit()
        close_oracle(con, curs)
...
# get_new_labor_contract(10001475, contract_ukid='51817400000009165')
